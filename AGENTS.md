...
- The playable game lives in web/; core rules are tested with npm test. The old prototype is preserved in legacy/index.html. Rebuild the standalone root index.html with python3 tools/build-standalone.py after changing web sources. The ROM lab lives in tools/rom-lab and opens as a local index.html without a server.
